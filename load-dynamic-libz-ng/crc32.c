...
//
// this will then be used for the crc32 function in this crate. This command only works 
// on x86_64 systems that support the pclmulqdq instruction.
#include <immintrin.h>
#include <wmmintrin.h>
...
#define PRINT(simd_value) { \
    long values[4] = { 0xAA }; \
    _mm_storeu_si128((__m128i*)values, simd_value); \
    printf("Values of xmm_variable: %ld %ld\n", values[0], values[1]); }
...
uint32_t crc32_sse(uint32_t crc32, const uint8_t *buf, size_t len) {
