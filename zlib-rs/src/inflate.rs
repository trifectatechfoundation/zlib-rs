                ...
                mode = 'blk: {
                    match mode {
                        Mode::Head => {
                            ...
                            if (self.wrap & 1) == 0
                                || ((self.bit_reader.bits(8) << 8) + (self.bit_reader.hold() >> 8))
                                    % 31
                                    != 0
                            {
                                mode = Mode::Bad;
                                ...
                                if extra_available > 0 {
                                    if let Some(head) = self.head.as_mut() {
                                        if !head.extra.is_null() {
                                            ...
                                            let written_so_far =
                                                head.extra_len as usize - self.length;
                            ...
                            if !cfg!(feature = "__internal-fuzz-disable-checksum") && self.wrap != 0
                            {
                                ...
                                if self.wrap & 4 != 0 {
                                    if self.gzip_flags != 0 {
                                        ...
                                        self.checksum =
                                            adler32(self.checksum, self.writer.filled());
                                    }
                                }
                                ...
                                let copy = if self.offset > copy {
                                    ...
                                    if copy > self.window.have() {
                                        if self.flags.contains(Flags::SANE) {
                                            mode = Mode::Bad;
                                            break 'label self
                                                .bad("invalid distance too far back\0");
                                        }
                        ...
                        Mode::Length => {
                            ...
                            if self.wrap != 0 && self.gzip_flags != 0 {
                                need_bits!(self, 32);
                                if (self.wrap & 4) != 0
                                    && self.bit_reader.hold() != self.total as u64
                                {
                                    mode = Mode::Bad;
