...
mod stable_api {
    ...
    fn with_window_bits(n: i32) {
        ...
        let mut inflate = zlib_rs::Decompress::new(n >= 0, n.abs() as u8);
        inflate
            .decompress(
                &compressed,
                &mut decompressed,
    ...
    fn invalid_config() {
        zlib_rs::Decompress::new(true, 123);
    }

    #[test]
    fn invalid_data() {
        let mut inflate = zlib_rs::Decompress::new(true, 15);
        ...
        assert_eq!(ret, Err(DecompressError::DataError));
    }
    ...
    fn need_dict() {
        ...
        assert_eq!(ret, Err(DecompressError::NeedDict { dict_id: 1 }));
    }
    ...
    fn reset_reuse() {
        ...
        let mut config1 = DeflateConfig::default();
        config1.window_bits = 15;
        let (compressed1, ret) = compress_slice(&mut output1, input1.as_bytes(), config1);
        assert_eq!(ret, ReturnCode::Ok);

        let mut output2 = [0u8; 64];
        let mut config2 = DeflateConfig::default();
        config2.window_bits = -15;
        ...
        let mut inflate = zlib_rs::Decompress::new(zlib_header_first, 15);
